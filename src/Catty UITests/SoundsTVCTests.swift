...
class SoundsTVCTests: XCTestCase, UITestProtocol {
    ...
    func testRenameAllSoundsFromBackground() {
        ...
        for i: UInt in 1...Constants.numSounds {
            ...
            XCTAssertTrue(tablesQuery.staticTexts[oldName].exists)
            XCTAssertFalse(tablesQuery.staticTexts[newName].exists)

            tablesQuery.staticTexts[oldName].swipeLeft()
            tablesQuery.buttons["More"].tap()
            app.buttons["Rename"].tap()
            clearTextButton.tap()
            app.alerts["Rename sound"].collectionViews.textFields["Enter your sound name here..."].typeText(newName)
            app.alerts["Rename sound"].collectionViews.buttons["OK"].tap()

            XCTAssertFalse(tablesQuery.staticTexts[oldName].exists)
            XCTAssertTrue(tablesQuery.staticTexts[newName].exists)
        }
    }
