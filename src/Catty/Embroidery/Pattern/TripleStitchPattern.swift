...
class TripleStitchPattern: StitchPatternProtocol {
    ...
    func spriteDidMove(to pos: CGPoint) {
        guard var lastPos = stream.last?.getPosition() else {
            fatalError("TripleStitch Mode running on empty EmbroideryStream. Should never happen")
        }
        ...
        while CGPoint.squaredDistance(from: lastPos, to: pos) > sqLen {
            ...
            let newPosition = lastPos + e * stitchLen
            addTripleStitch(from: lastPos, to: newPosition)
            lastPos = newPosition

        }
        addTripleStitch(from: lastPos, to: pos)
    }

    private func addTripleStitch(from lastPos: CGPoint, to newPos: CGPoint) {
        stream.add(Stitch(atPosition: newPos))
