...
class TermsOfUseOptionTableViewController: BOTableViewController {

    override func setup() {
        title = kLocalizedTermsOfUse
        view.backgroundColor = UIColor.background()
        view.tintColor = UIColor.globalTint()
        addSection(BOTableViewSection(headerTitle: "", handler: { section in
            section?.addCell(BOTableViewCell(title: kLocalizedTermsOfUseDescription, key: nil, handler: { cell in
                let sectionCell = cell as! BOTableViewCell?
                sectionCell?.backgroundColor = UIColor.background()
            }))
            section?.addCell(BOButtonTableViewCell(title: kLocalizedTermsOfUse, key: nil, handler: { cell in
