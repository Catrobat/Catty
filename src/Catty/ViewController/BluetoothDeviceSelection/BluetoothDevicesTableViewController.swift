...
class BluetoothDevicesTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        self.stagePresenterVC = StagePresenterViewController()
    ...
    func deviceConnected(_ peripheral: Peripheral) {
        if let delegate = self.delegate, let deviceArray = delegate.deviceArray, !deviceArray.isEmpty {
    ...
    func startScan() {
        let afterPeripheralDiscovered = {(_: Peripheral) in
            self.updateWhenActive()
        }
        let afterTimeout = {(_: NSError) in
        }
    ...
    func checkStart() {
        ...
        if delegate != nil && delegate!.deviceArray != nil && !delegate!.deviceArray!.isEmpty {
            delegate?.setHeader()
            return
        }
