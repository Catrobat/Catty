...
@objcMembers class VisualPlacementViewController: UIViewController {
    ...
    private var cachedLook: ObjectLook!
    ...
    func initWith(brick: BrickVisualPlacementProtocol, andObject object: SpriteObject) {
        ...
        self.cachedLook = ObjectCache.shared.getLook(for: object.name)
        if self.cachedLook.lastLook == nil {
            self.cachedLook.lastLook = self.objectImage
        } else { self.objectImage = self.cachedLook.lastLook }
    }
    ...
    func setupObjectImageView(_ imageView: UIImageView) {
        if var objectImage = objectImage {
            var catrobatScaling = 1.0
            if let scaling = cachedLook?.size {
                catrobatScaling = scaling
            }
            catrobatScaling = catrobatScaling == 0 ? 1.0 : catrobatScaling
            objectImage = objectImage.rotated(byDegrees: self.cachedLook.rotation - 90.0)
