...
import Foundation
class TextBlocksNumberSensor: DeviceDoubleSensor {

    static let tag = "TEXT_BLOCKS_NUMBER"
    static let name = kUIFESensorTextBlocksNumber
    ...
    func rawValue(landscapeMode: Bool) -> Double {
        guard let textBlocksNumber = self.getVisualDetectionManager()?.textBlocksNumber else { return 0.0 }
        return Double(textBlocksNumber)
    }
