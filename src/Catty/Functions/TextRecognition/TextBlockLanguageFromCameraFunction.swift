...
class TextBlockLanguageFromCameraFunction: SingleParameterStringFunction {

    static let tag = "TEXT_BLOCK_LANGUAGE_FROM_CAMERA"
    static let name = kUIFEFunctionTextBlockLanguageFromCamera
    ...
    func value(parameter: AnyObject?) -> String {
        ...
        if textBlockNumber <= 0 || textBlockNumber > visualDetectionManager.textBlockLanguageCode.count {
            return type(of: self).defaultValue
        }

        return visualDetectionManager.textBlockLanguageCode[textBlockNumber - 1]
    }
