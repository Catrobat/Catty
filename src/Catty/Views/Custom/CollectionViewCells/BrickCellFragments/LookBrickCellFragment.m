...
- (instancetype)initWithFrame:(CGRect)frame AndBrickCell:(BrickCell*)brickCell
{
    if(self = [super initWithFrame:frame]) {
        ...
        int optionIndex = 1;
        if([brickCell.scriptOrBrick isKindOfClass:[SetLookBrick class]]) {
            SetLookBrick *lookBrick = (SetLookBrick*)brickCell.scriptOrBrick;
            for(Look *look in lookBrick.script.object.lookList) {
                [options addObject:look.name];
                if([look.name isEqualToString:lookBrick.look.name])
