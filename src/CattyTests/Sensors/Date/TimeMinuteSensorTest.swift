...
final class TimeMinuteSensorTest: XCTestCase {
    ...
    override func setUp() {
        self.sensor = TimeMinuteSensorMock()
    }
    ...
    func testTag() {
        XCTAssertEqual("TIME_MINUTE", type(of: sensor).tag)
    }
    ...
    func testFormulaEditorSection() {
        XCTAssertEqual(.device(position: 280), type(of: sensor).formulaEditorSection(for: SpriteObject()))
    }
}
