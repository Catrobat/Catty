...
final class GlideToBrickCellTests: XCTestCase {

    var brick: GlideToBrick!
    var brickCell: GlideToBrickCell!

    override func setUp() {
        super.setUp()

        brick = GlideToBrick()
        brickCell = GlideToBrickCell()
        brickCell.scriptOrBrick = brick
    }

    func testTitleSingular() {
        let expectedTitle = kLocalizedGlide + " %@ " + kLocalizedSecond + "\n" + kLocalizedToX + " %@ " + kLocalizedYLabel + " %@"
    ...
    func testTitlePlural() {
        let expectedTitle = kLocalizedGlide + " %@ " + kLocalizedSeconds + "\n" + kLocalizedToX + " %@ " + kLocalizedYLabel + " %@"
