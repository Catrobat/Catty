...
class SceneTests: XCTestCase {
    ...
    func testTouchConversionCenter() {
        let scaledScene: CBScene? = (SceneBuilder(project: ProjectMock(width: screenSize.width * 2, andHeight: screenSize.height * 2))).build()
    ...
    func testTouchConversionCenterNoScale() {
        let scaledScene: CBScene? = SceneBuilder(project: ProjectMock(width: screenSize.width, andHeight: screenSize.height)).build()
        let scaledSceneCenter = CGPoint(x: screenSize.width / 2, y: screenSize.height / 2)
        let convertedCenter: CGPoint = CBSceneHelper.convertTouchCoordinateToPoint(coordinate: scaledSceneCenter, sceneSize: (scaledScene?.size)!)
    ...
    func testTouchConversionBottomLeft() {
        ...
        let pocketCodeBottomLeft = CGPoint(x: scaledScene?.size.width ?? 0 / 2 * -1, y: scaledScene?.size.height ?? 0 / 2 * -1)
    ...
    func testTouchConversionBottomRight() {
        ...
        let pocketCodeBottomRight = CGPoint(x: scaledScene?.size.width ?? 0 / 2, y: scaledScene?.size.height ?? 0 / 2 * -1)
    ...
    func testTouchConversionTopLeft() {
        ...
        let pocketCodeTopLeft = CGPoint(x: scaledScene?.size.width ?? 0 / 2 * -1, y: scaledScene?.size.height ?? 0 / 2)
    ...
    func testTouchConversionTopRight() {
        ...
        let pocketCodeTopRight = CGPoint(x: (scaledScene?.size.width ?? 0 / 2), y: (scaledScene?.size.height ?? 0 / 2))
