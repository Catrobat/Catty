...
final class XMLSerializerTests: XMLAbstractTest {
    ...
    func testHeader() {
        let project = self.getProjectForXML(xmlFile: "ValidHeader0999")
        let header = project.header
        let equal = self.isXMLElement(xmlElement: header.xmlElement(with: nil), equalToXMLElementForXPath: "//program/header", inProjectForXML: "ValidHeader0999")
    ...
    func testRemoveObjectAndSerializeProject() {
        let projectName = "ValidProjectAllBricks0999"
        ...
        let parserContext = CBXMLParserContext(languageVersion: CGFloat(Float32(0.999)), andRootElement: xmlElement)
    ...
    func testUserVariables() {
        let project = self.getProjectForXML(xmlFile: "UserVariables_0999")
