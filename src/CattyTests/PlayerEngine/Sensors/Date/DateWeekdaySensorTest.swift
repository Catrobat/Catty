...
final class DateWeekdaySensorTest: XCTestCase {
    ...
    func testRawValue() {
        /* test Sunday */
        sensor.mockDate = Calendar.current.date(from: DateComponents(year: 2_018, month: 6, day: 17, hour: 10))!
        XCTAssertEqual(1, Int(sensor.rawValue()))

        /* test Monday */
        sensor.mockDate = Calendar.current.date(from: DateComponents(year: 2_018, month: 6, day: 18, hour: 10))!
        XCTAssertEqual(2, Int(sensor.rawValue()))

        /* test Tuesday */
        sensor.mockDate = Calendar.current.date(from: DateComponents(year: 2_018, month: 6, day: 19, hour: 10))!
        XCTAssertEqual(3, Int(sensor.rawValue()))

        /* test Wednesday */
        sensor.mockDate = Calendar.current.date(from: DateComponents(year: 2_018, month: 6, day: 20, hour: 10))!
        XCTAssertEqual(4, Int(sensor.rawValue()))

        /* test Thursday */
        sensor.mockDate = Calendar.current.date(from: DateComponents(year: 2_018, month: 6, day: 21, hour: 10))!
        XCTAssertEqual(5, Int(sensor.rawValue()))

        /* test Friday */
        sensor.mockDate = Calendar.current.date(from: DateComponents(year: 2_018, month: 6, day: 22, hour: 10))!
        XCTAssertEqual(6, Int(sensor.rawValue()))

        /* test Saturday */
        sensor.mockDate = Calendar.current.date(from: DateComponents(year: 2_018, month: 6, day: 23, hour: 10))!
        XCTAssertEqual(7, Int(sensor.rawValue()))

        /* test edge case - almost the beginning of the next day - Tuesday */
        sensor.mockDate = Calendar.current.date(from: DateComponents(year: 2_018, month: 6, day: 19, hour: 23))!
        XCTAssertEqual(3, Int(sensor.rawValue()))
    }
